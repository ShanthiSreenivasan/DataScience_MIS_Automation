...
Mdf1 <- read.xlsx("C:\\R\\Lender Feedback\\Input\\Muthoot.xlsx",sheet = 'sep')# %>% select(Mobile.Number,Feedback)

colnames(Mdf1)[which(names(Mdf1) == "Mobile Number")] <- "Mobile.Number"

Mdf1<-Mdf1 %>% select(Mobile.Number,Feedback)
...
Mdf2 <- read.xlsx("C:\\R\\Lender Feedback\\Input\\Muthoot.xlsx",sheet = 'oct')# %>% select(Mobile.Number,Feedback)

colnames(Mdf2)[which(names(Mdf2) == "Mobile Number")] <- "Mobile.Number"

Mdf2<-Mdf2 %>% select(Mobile.Number,Feedback)
...
Muthoot_appos_dump1 <- read.csv("C:\\R\\Lender Feedback\\Input\\appopsdump_1.csv") %>% filter(grepl('Muthoot',name,ignore.case=TRUE)) %>% select(phone_home,offer_application_number,status,name,appops_status_code)
Muthoot_appos_dump2 <- read.csv("C:\\R\\Lender Feedback\\Input\\appopsdump_2.csv") %>% filter(grepl('Muthoot',name,ignore.case=TRUE)) %>% select(phone_home,offer_application_number,status,name,appops_status_code)
Muthoot_appos_dump<-rbind(Muthoot_appos_dump1,Muthoot_appos_dump2)
...
colnames(Muthoot_lender_MIS)[which(names(Muthoot_lender_MIS) == "Feedback")] <- "customer_status_name"
...
Muthoot_lender_MIS$Remark[Muthoot_lender_MIS$Remark == 'Repeated_Feedback_cases' & grepl('80',Muthoot_lender_MIS$New_appops_status,ignore.case=TRUE) & Muthoot_lender_MIS$CURRENT_appops_status %in% c(89,140,150,210,212,213,250,270,272,
                                                                                                                                                                                                           300,320,350,360,370,382,383,390,393,394,395,399)] <- 380 #, Muthoot_lender_MIS$New_appops_status %in% c(280,380) 

Muthoot_lender_MIS$Remark[Muthoot_lender_MIS$Remark == 'Repeated_Feedback_cases' & grepl('80',Muthoot_lender_MIS$New_appops_status,ignore.case=TRUE) & Muthoot_lender_MIS$CURRENT_appops_status %in% c(400,401,420,421,425,450,460,470,490,491,494,495)] <- 480 #, Muthoot_lender_MIS$New_appops_status %in% c(480) 

Muthoot_lender_MIS$Remark[Muthoot_lender_MIS$Remark == 'Repeated_Feedback_cases' & grepl('80',Muthoot_lender_MIS$New_appops_status,ignore.case=TRUE) & Muthoot_lender_MIS$CURRENT_appops_status %in% c(500,520,550,560,570,590)] <- 580#, Muthoot_lender_MIS$New_appops_status %in% c(580) 

Muthoot_lender_MIS$Remark[Muthoot_lender_MIS$Remark == 'Repeated_Feedback_cases' & grepl('80',Muthoot_lender_MIS$New_appops_status,ignore.case=TRUE) & Muthoot_lender_MIS$CURRENT_appops_status %in% c(690,650,660,670,700,701,780)] <- 680#, Muthoot_lender_MIS$New_appops_status %in% c(680) 

colnames(Muthoot_lender_MIS)[which(names(Muthoot_lender_MIS) == "offer_application_number")] <- "Application_Number"

Muthoot_lender_MIS<-Muthoot_lender_MIS %>% mutate('Lender'="Muthoot",`loan_amount`='',`Customer_requested_loan_amount`='')
